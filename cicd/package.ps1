...
try {
    ...
    $buildSpec = docker compose $buildComposeFiles config --format json | ConvertFrom-Json
    $releaseSpec = docker compose $releaseComposeFiles config --format json | ConvertFrom-Json

    foreach ($component in $components) {
        $source = ($buildSpec.services.PSObject.properties | where { $_.Name -eq $component }).Value.image
        $target = ($releaseSpec.services.PSObject.properties | where { $_.Name -eq $component }).Value.image
